...
<head>
    ...
    <nav class="navbar navbar-light bg-white shadow-sm mb-4 px-4">
        <div class="container-fluid d-flex justify-content-between">
            <a href="{{ route('contratos.listar') }}" class="btn btn-outline-danger">← Voltar</a>
            <a class="btn btn-outline-danger" href="{{ route('sair') }}">Sair</a>
        </div>
    </nav>

        <div class="container w-full max-w-2xl pt-4">
            <h1 class="text-2xl font-bold text-center mb-6 text-red-700">{{ $nome }}</h1>

            @if(empty($faturas))
                <div class="alert alert-warning">Erro: Nenhum dado encontrado.</div>
            @else
                <div class="p-4 bg-white rounded shadow">
                    <h1 class="text-2xl font-bold text-center mb-6 text-red-700">Dados do Plano</h1>
                    <hr class="mb-4">
                    <div class="mb-4 space-y-2">
                        ...
                        @if ($qntd_visitas > 0)
                            <div>
                                <label class="block text-red-800 font-semibold">Qntd. de visitas técnicas</label>
                                <input type="text" nome="cliente_nome" value="{{ $qntd_visitas }}" class="w-full px-2 py-2 text-center rounded bg-gray-200" disabled>
                            </div>
                        @endif
                        @if ($qntd_atrasos > 0)
                            <div>
                                <label class="block text-red-800 font-semibold">Qntd. de atrasos</label>
                        ...
                        @endif
                            <div>
                                <label class="block text-red-800 font-semibold">Receita media</label>
                        ...
                        @if($retencao == 'Não')
                            <div>
                                <label class="block text-red-800 font-semibold">Reter cliente:</label>
                        ...
                        @else
                            ...
                            <hr class="my-4">
                            <h1 class="text-2xl font-bold text-center mb-6 text-red-700">Opções disponiveis</h1>
                            @if ($mensalidades > 0)
                                <div>
                                    <label class="block text-red-800 font-semibold">Mensalidades Grátis</label>
                            ...
                            @if ($movel_gratis > 0)
                                <div>
                                    <label class="block text-red-800 font-semibold">Plano móvel (valor de R$ 39,90)</label>
                            ...
                            @if ($sva_gratis > 0)
                                <div>
                                    <label class="block text-red-800 font-semibold">SVA (valor de R$ 44,90)</label>
